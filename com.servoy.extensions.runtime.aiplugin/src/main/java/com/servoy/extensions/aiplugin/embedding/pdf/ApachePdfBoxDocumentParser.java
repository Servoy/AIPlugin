package com.servoy.extensions.aiplugin.embedding.pdf;
 ...
 */
public record ApachePdfBoxDocumentParser(boolean includeMetadata) implements DocumentParser {
		...
		case IFile file -> {
			try (InputStream is = file.getInputStream()) {
				...
				if (file.getFile() != null)
					document.metadata().put("filename", file.getFile().getName());
//				document.metadata().put("filename", file.getName());
				return document;
			} catch (IOException e) {
				throw new RuntimeException(e);
